 ...
 *  2. Öppna matspar.se/start med strängen som query-param (?mk_cart=...).
 *     /start (till skillnad från "/") kräver inte att postnummer/leveransområde
 *     väljs innan sidan visas – det får användaren istället ange som ett
 *     vanligt steg i Matspars egen utcheckning.
...
export function buildMatsparOpenUrl(cartParam: string): string {
  const url = new URL("https://www.matspar.se/start");
